-- ============================================================
-- NurseArena — Complete Database Schema
...
create table if not exists questions (
  ...
  correctAnswers jsonb default '[]'::jsonb,
  category text check (category in ('I','II')),
  options jsonb,
  explanation text,
  ...
  content_hash text generated always as (md5(lower(trim(coalesce(question, ''))))) stored,
...
-- 7. QUIZ ATTEMPTS (per-quiz summary)
...
create table if not exists live_quiz_events (
  ...
  status varchar(16) default 'scheduled' check (status in ('scheduled','live','catchup','closed','failed','cancelled')),
  scoring_profile_id uuid references quiz_scoring_profiles(id),
  current_q_index smallint default 0
);

-- 22. LIVE QUIZ PER-USER RESULTS
...
-- 23. ANSWER SUBMISSIONS (LIVE QUIZ)
...
create table if not exists notifications (
  ...
  title text not null,
  body text,
  type text check (type in ('mock_test','result','announcement')),
  targetExams jsonb default '[]'::jsonb,
  createdAt timestamptz default now(),
  readBy jsonb default '[]'::jsonb
);

-- 26. QUIZ POOL SUMMARY VIEW
...
-- 27. LEADERBOARD MATERIALIZED VIEW (daily)
...
select
  ...
  rank() over (
    ...
    order by sum(case when a.is_correct then 10 else 0 end) desc,
...
-- Quiz answers: users manage their own
...
-- Quiz sessions: users manage their own
create policy "Users manage own sessions"
  on quiz_sessions for all using (auth.uid() = user_id);

-- Session answers: users manage their own (via session ownership)
...
-- Admin: manage admin-only resources
...
create policy "Admin manage mock test events"
  on mock_test_events for all using (
    exists (select 1 from profiles where id = auth.uid() and is_admin = true)
  );
create policy "Admin manage live quiz events"
...
create policy "Admin manage missions"
  on missions for all using (
    exists (select 1 from profiles where id = auth.uid() and is_admin = true)
  );

-- Reference tables: authenticated can read
...
-- User missions: users manage their own
